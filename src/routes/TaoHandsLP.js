...
class Collapsible extends Component {
	...
	render() {
		...
		return (<div style={ClickButton}>
			<div onClick={(e)=>this.panelClicked(e)} style={ClickHere}>
				{this.props.title}
			</div>
			{(this.state.open==='half') ? (
				<div className='content'>
					{this.props.children}
					{this.props.registered ? 
						<div>
							<p><strong>Thank you for registering for this event!</strong></p>
						</div> : null}
				</div>
				) 
				: 
...
export default class TaoHandsLP extends Component {
	constructor(props){
		...
		this.state = {
				//form: 'login', 
				email: '', firstName: '', lastName: '', registered: false
		}
	};
	TaoHandsLPregister = (event) => {
		...
		this.props.socket.emit('client:TaoHandsLPregister', messageObject);
	...
	render({ url }, { open }) {
		const buttonC = {
			...
			backgroundColor: 'coral',//'PeachPuff',//SandyBrown',
			padding: '20px'
		}
		const button = {
			margin: 'auto', //center the div
		...
		return (
			<div>
				...
				{/*<Collapsible title="Click Here To Register For Free" socket={this.props.socket}>
					<div>
						<form onSubmit={this.TaoHandsLPregister}>
							...
							<p><input type='text' placeholder='email' name='email' required
									onChange={this.changeHandler}/></p>
							<p><button style="background-color: pink">Register Me!</button></p>
						</form>
				...
				<div style="text-align:center">
					...
					<h2>Saturday June 22, 10am - noon<br />
						Venue: Master Sha Tao Center, 1549 California St, San Francisco</h2>
				</div>
				...
				<Collapsible title="Click Here To Register For Free" socket={this.props.socket}
					...
					<div>
						<form onSubmit={this.TaoHandsLPregister}>
							...
							<p><input type='text' placeholder='email' name='email' required
									onChange={this.changeHandler}/></p>
							<p><button style="background-color: pink"><strong>Register Me!</strong></button></p>
						</form>
						{this.props.errSignup}
					</div>
				</Collapsible>

			<p style={p}>Act Fast - Space is limited and will fill up quickly</p>
